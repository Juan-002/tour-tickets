  ...
  const stats = [
    {
      ...
      icon: (
        <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#C4903E" strokeWidth="1.5">
  ...
  return (
    <div className="flex flex-col gap-8 anim-fade-up">
      ...
      <div>
        ...
        <p className="mt-2 font-sans text-sm text-white/35">
      ...
      <div className="grid grid-cols-4 gap-4">
        {stats.map((s, i) => (
          <div
            key={s.label}
            className="bg-white/[0.03] border border-white/[0.07] p-6 hover:border-white/[0.12] transition-colors duration-300 anim-fade-up"
            style={{ animationDelay: `${i * 0.08}s`, opacity: 0 }}
          >
            <div className="flex items-start justify-between mb-4">
              <div
                className="flex items-center justify-center w-11 h-11 shrink-0"
                style={{ border: `1px solid ${s.accent}25` }}
              >
                {s.icon}
              </div>
              <span
                className="text-[0.6rem] tracking-widest uppercase font-sans px-2 py-1 border"
                style={{ color: s.accent, borderColor: `${s.accent}25`, background: `${s.accent}10` }}
              >
                Live
              </span>
            </div>
            <p className="font-serif text-3xl font-black" style={{ color: s.accent }}>
              {s.value}
            </p>
            <p className="text-[0.68rem] text-white/40 font-sans mt-1 tracking-wide">{s.label}</p>
            <p className="text-[0.65rem] text-white/25 font-sans mt-0.5">{s.sub}</p>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <div className="bg-white/[0.02] border border-white/[0.07]">
          <div className="flex items-center justify-between px-6 py-4 border-b border-white/[0.06]">
            ...
            <a
              href="/admin/tickets"
              className="text-[0.68rem] text-[#C4903E] hover:underline font-sans tracking-wide"
            >
              Ver todas →
            </a>
          </div>
          ...
          {recentTickets.length === 0 ? (
            <div className="px-6 py-10 font-sans text-sm text-center text-white/25">
              No hay ventas aún
            </div>
          ) : (
            <div className="divide-y divide-white/[0.04]">
              {recentTickets.map((ticket) => {
                ...
                return (
                  <div key={ticket.id} className="flex items-center justify-between px-6 py-3 hover:bg-white/[0.02] transition-colors">
                    <div className="min-w-0">
                      <p className="font-sans text-sm truncate text-white/80">
                        {tour?.title ?? "Tour"}
                      </p>
                      <p className="font-sans text-xs text-white/30 mt-0.5">
                        {ticket.quantity} persona{ticket.quantity > 1 ? "s" : ""}
                      </p>
                    </div>
                    <div className="flex items-center gap-3 ml-4 shrink-0">
                      <p className="font-serif text-sm font-bold text-[#C4903E]">
                        ${ticket.totalPrice.toLocaleString("es-CO")}
                      </p>
                      <span className={`text-[0.6rem] tracking-widest uppercase font-sans px-2 py-0.5 ${ticketStatusStyle[ticket.status]}`}>
          ...
          {recentTours.length === 0 ? (
            ...
            <div className="divide-y divide-white/[0.04]">
              {recentTours.map((tour) => (
                <div key={tour.id} className="flex items-center justify-between px-6 py-3 hover:bg-white/[0.02] transition-colors">
                  <div className="min-w-0">
                    <p className="font-sans text-sm truncate text-white/80">{tour.title}</p>
                    <p className="font-sans text-xs text-white/30 mt-0.5">
                      {tour.availableSlots} / {tour.totalSlots} cupos
                    </p>
                  </div>
                  <div className="flex items-center gap-3 ml-4 shrink-0">
                    <p className="font-serif text-sm font-bold text-[#C4903E]">
                    ...
                    <span className={`text-[0.6rem] tracking-widest uppercase font-sans px-2 py-0.5 ${tourStatusStyle[tour.status]}`}>
      ...
      <div className="border border-white/[0.06] p-6">
        <p className="text-[0.68rem] tracking-[0.2em] uppercase text-white/35 font-sans mb-4">
          Acciones rápidas
        </p>
        <div className="flex gap-3">
          <a href="/admin/tours/new" className="btn-primary" style={{ padding: "10px 24px" }}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              <line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/>
            </svg>
            Nuevo tour
          </a>
          <a href="/tours" target="_blank" className="btn-ghost" style={{ padding: "10px 24px" }}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
